#!/usr/bin/env python3
"""One-time backfill: scan BabyandBump forums from Jan 1, 2026 to today.

Deep-paginates all configured BabyandBump forums, applies search term
filtering, scoring, risk assessment, post-type classification, gender
inference, and draft reply generation. Results go to the Review Queue tab
on the outreach Google Sheet (outreach_status='review').

Uses the full agent stack so all pipeline logic matches the regular cron.

Usage:
    python scripts/backfill_babyandbump.py
    python scripts/backfill_babyandbump.py --max-pages 50   # limit pages per forum
    python scripts/backfill_babyandbump.py --dry-run         # scan only, no Sheet sync
"""
...
logger = logging.getLogger("backfill_babyandbump")

WORKSPACE_ID = "dsdn"
CUTOFF_DATE = "2026-01-01"
DEFAULT_MAX_PAGES = 100  # pages per forum, ~20 threads per page


async def main() -> None:
    import argparse

    parser = argparse.ArgumentParser(description="Backfill BabyandBump outreach scan")
    parser.add_argument(
    ...
    try:
        ...
        # ── Step 2: Deep-paginate BabyandBump forums ──────────────
        logger.info(
            "Step 2: Scanning BabyandBump (max_pages=%d, cutoff=%s)...",
            args.max_pages, CUTOFF_DATE,
        )
        bab_result = await scanner._scan_babyandbump(
            WORKSPACE_ID,
            max_pages=args.max_pages,
            cutoff_date=CUTOFF_DATE,
        )
        logger.info(
            "BabyandBump scan complete: scanned=%d new=%d errors=%s",
            bab_result.get("scanned", 0),
            bab_result.get("new", 0),
            bab_result.get("errors", []),
        )

        if bab_result.get("new", 0) == 0:
        ...
        # ── Summary ───────────────────────────────────────────────
        print(f"\n{'='*60}")
        print("BabyandBump Backfill Complete!")
        from cmo_agent.agents.outreach_scanner import _BAB_FORUM_SLUGS
        print(f"  Forums scanned: {len(_BAB_FORUM_SLUGS)}")
        print(f"  Pages per forum: up to {args.max_pages}")
        print(f"  Cutoff date: {CUTOFF_DATE}")
        print(f"  Threads scanned: {bab_result.get('scanned', 0)}")
        print(f"  New posts found: {bab_result.get('new', 0)}")
        ...
        if bab_result.get("errors"):
            print(f"  Errors: {bab_result['errors']}")
        print(f"{'='*60}")
