#!/usr/bin/env python3
"""One-time backfill: scan Healthboards.com DS board from Jan 1, 2026 to today.

Deep-paginates the Healthboards Down Syndrome forum (vBulletin 3.9.3),
applies search term filtering, scoring, risk assessment, post-type
classification, gender inference, and draft reply generation. Results go
to the Review Queue tab on the outreach Google Sheet (outreach_status='review').

Uses the full agent stack so all pipeline logic matches the regular cron.

Usage:
    python scripts/backfill_healthboards.py
    python scripts/backfill_healthboards.py --max-pages 10   # limit pages
    python scripts/backfill_healthboards.py --dry-run         # scan only, no Sheet sync
"""
...
logger = logging.getLogger("backfill_healthboards")

WORKSPACE_ID = "dsdn"
CUTOFF_DATE = "2026-01-01"
DEFAULT_MAX_PAGES = 15  # pages per forum, ~20 threads per page


async def main() -> None:
    import argparse

    parser = argparse.ArgumentParser(description="Backfill Healthboards outreach scan")
    parser.add_argument(
        "--max-pages",
        type=int,
    ...
    try:
        ...
        # ── Step 2: Deep-paginate Healthboards forums ─────────────
        logger.info(
            "Step 2: Scanning Healthboards (max_pages=%d, cutoff=%s)...",
            args.max_pages,
            CUTOFF_DATE,
        )
        hb_result = await scanner._scan_healthboards(
            WORKSPACE_ID,
            max_pages=args.max_pages,
            cutoff_date=CUTOFF_DATE,
        )
        logger.info(
            "Healthboards scan complete: scanned=%d new=%d errors=%s",
            hb_result.get("scanned", 0),
            hb_result.get("new", 0),
            hb_result.get("errors", []),
        )

        if hb_result.get("new", 0) == 0:
        ...
        # ── Summary ───────────────────────────────────────────────
        print(f"\n{'=' * 60}")
        print("Healthboards Backfill Complete!")
        from cmo_agent.agents.outreach_scanner import _HB_FORUM_SLUGS

        print(f"  Forums scanned: {len(_HB_FORUM_SLUGS)}")
        print(f"  Pages per forum: up to {args.max_pages}")
        print(f"  Cutoff date: {CUTOFF_DATE or 'none (all threads)'}")
        print(f"  Threads scanned: {hb_result.get('scanned', 0)}")
        print(f"  New posts found: {hb_result.get('new', 0)}")
        ...
        if hb_result.get("errors"):
            print(f"  Errors: {hb_result['errors']}")
        print(f"{'=' * 60}")
