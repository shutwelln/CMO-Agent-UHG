#!/usr/bin/env python3
"""One-time backfill: scan TheBump forums from Jan 1, 2026 to today.

Deep-paginates all configured TheBump Vanilla Forums categories, applies
search term filtering, scoring, risk assessment, post-type classification,
gender inference, and draft reply generation. Results go to the Review Queue
tab on the outreach Google Sheet (outreach_status='review').

Uses the full agent stack so all pipeline logic matches the regular cron.

Usage:
    python scripts/backfill_thebump.py
    python scripts/backfill_thebump.py --max-pages 100  # limit pages per forum
    python scripts/backfill_thebump.py --dry-run          # scan only, no Sheet sync
"""
...
logger = logging.getLogger("backfill_thebump")

WORKSPACE_ID = "dsdn"
CUTOFF_DATE = "2026-01-01"
DEFAULT_MAX_PAGES = 100  # pages per forum


async def main() -> None:
    import argparse

    parser = argparse.ArgumentParser(description="Backfill TheBump outreach scan")
    parser.add_argument(
    ...
    try:
        ...
        # ── Step 2: Deep-paginate TheBump forums ──────────────────
        logger.info(
            "Step 2: Scanning TheBump (max_pages=%d, cutoff=%s)...",
            args.max_pages, CUTOFF_DATE,
        )
        tb_result = await scanner._scan_thebump(
            WORKSPACE_ID,
            max_pages=args.max_pages,
            cutoff_date=CUTOFF_DATE,
        )
        logger.info(
            "TheBump scan complete: scanned=%d new=%d errors=%s",
            tb_result.get("scanned", 0),
            tb_result.get("new", 0),
            tb_result.get("errors", []),
        )

        if tb_result.get("new", 0) == 0:
        ...
        # ── Summary ───────────────────────────────────────────────
        from cmo_agent.agents.outreach_scanner import _THEBUMP_FORUM_SLUGS

        print(f"\n{'='*60}")
        print("TheBump Backfill Complete!")
        print(f"  Forums scanned: {len(_THEBUMP_FORUM_SLUGS)}")
        print(f"  Pages per forum: up to {args.max_pages}")
        print(f"  Cutoff date: {CUTOFF_DATE}")
        print(f"  Threads scanned: {tb_result.get('scanned', 0)}")
        print(f"  New posts found: {tb_result.get('new', 0)}")
        ...
        if tb_result.get("errors"):
            print(f"  Errors: {tb_result['errors']}")
        print(f"{'='*60}")
