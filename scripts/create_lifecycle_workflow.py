"""Create the 'Saverwell Lifecycle Subscribe' n8n workflow.

Copies the existing 'Saverwell Email Subscribe' workflow structure and adds
a Code node for lifecycle computed attributes (content_interest, state_resolved,
lifecycle_stage, welcome_flow_started, articles_received).

Usage:
    python scripts/create_lifecycle_workflow.py
"""
...
# ZIP-to-state lookup JS code for the n8n Code node
LIFECYCLE_CODE_JS = r"""// Compute lifecycle attributes from subscriber data
const body = $('Subscribe Webhook').item.json.body || {};
const url = body.url || '';
const zip = body.utm_zip_code || body.zip || '';

// --- content_interest: derive from signup page URL ---
let content_interest = 'all';
if (url.includes('/protection')) {
  content_interest = 'protection';
} else if (url.includes('/guide') || url.includes('/medicare')) {
  content_interest = 'guides';
} else if (url.includes('/discount') || url.includes('/savings')) {
  content_interest = 'savings';
}

// --- state_resolved: ZIP prefix to state lookup ---
const ZIP_TO_STATE = {
...
if (zip && zip.length >= 3) {
  ...
  state_resolved = ZIP_TO_STATE[prefix] || '';
}

// --- Lifecycle defaults ---
const lifecycle_stage = 'new';
const welcome_flow_started = false;
const articles_received = 0;
const articles_received_list = '';

// Pass through all existing fields + add computed attributes
return [{
  json: {
    ...$input.item.json,
    content_interest,
...
async def main() -> None:
    ...
    async with client:
        # Build the workflow payload — mirrors existing structure with lifecycle additions
        workflow_payload = {
            "name": "Saverwell Lifecycle Subscribe",
            "nodes": [
                {
                    ...
                    "parameters": {
                        "conditions": {
                            ...
                            "conditions": [
                                {
                                    "id": "token-check",
                                    "leftValue": '={{$json.headers["x-saverwell-token"]}}',
                                    "rightValue": "Zv4wfQhFLHjXaD2Dppvj3W6EU2Wi",
                                    "operator": {
                                        "type": "string",
                                        "operation": "equals",
                ...
                {
                    "name": "Successful - Set Data Fields",
                ...
                # NEW NODE: Compute Lifecycle Attributes
                {
                    "name": "Compute Lifecycle Attributes",
                    "type": "n8n-nodes-base.code",
                    "typeVersion": 2.0,
                    "position": [24, 16],
                    "parameters": {"jsCode": LIFECYCLE_CODE_JS},
                },
                {
                    ...
                    "parameters": {
                        ...
                        "columns": {
                            ...
                            "value": {
                                ...
                                "ip_address": "={{ $('Subscribe Webhook').item.json.headers['x-real-ip'] }}",
                                "full_json": "={{ $('Subscribe Webhook').item.json.body }}",
                                "utm_source": "={{ $('Subscribe Webhook').item.json.body.utm_source }}",
                                "email": "={{ $('Subscribe Webhook').item.json.body.email }}",
                                "utm_campaign": "={{ $('Subscribe Webhook').item.json.body.utm_campaign }}",
                                "utm_content": "={{ $('Subscribe Webhook').item.json.body.utm_content }}",
                                "utm_medium": "={{ $('Subscribe Webhook').item.json.body.utm_medium }}",
                                "utm_referrer": "={{ $('Subscribe Webhook').item.json.body.utm_referrer }}",
                                "referring_url": "={{ $('Subscribe Webhook').item.json.body.referrer }}",
                                "brand": "={{ $('Subscribe Webhook').item.json.body.brand }}",
                                "utm_leadid": "={{ $('Verify Webhook Request').item.json.body.utm_leadid }}",
                                "utm_subid": "={{ $('Verify Webhook Request').item.json.body.utm_subid }}",
                                "utm_zip_code": "={{ $('Verify Webhook Request').item.json.body.utm_zip_code }}",
                    ...
                    "parameters": {
                        ...
                        "fieldsUi": {
                            "fieldValues": [
                                ...
                                {"fieldId": "brand", "fieldValue": "={{ $json.full_json.brand }}"},
                                {"fieldId": "consent", "fieldValue": "={{ $json.full_json.consent }}"},
                                {"fieldId": "source", "fieldValue": "={{ $json.full_json.source }}"},
                                {"fieldId": "site", "fieldValue": "={{ $json.full_json.site }}"},
                                {"fieldId": "url", "fieldValue": "={{ $json.full_json.url }}"},
                                {"fieldId": "referrer", "fieldValue": "={{ $json.full_json.referrer }}"},
                                {"fieldId": "utm_source", "fieldValue": "={{ $json.full_json.utm_source }}"},
                                {"fieldId": "utm_campaign", "fieldValue": "={{ $json.full_json.utm_campaign }}"},
                                {"fieldId": "utm_medium", "fieldValue": "={{ $json.full_json.utm_medium }}"},
                                {"fieldId": "utm_content", "fieldValue": "={{ $json.full_json.utm_content }}"},
                                {"fieldId": "utm_referrer", "fieldValue": "={{ $json.full_json.utm_referrer }}"},
                                {"fieldId": "utm_zip_code", "fieldValue": "={{ $json.full_json.utm_zip_code }}"},
                                {"fieldId": "utm_leadid", "fieldValue": "={{ $json.full_json.utm_leadid }}"},
                                {"fieldId": "utm_subid", "fieldValue": "={{ $json.full_json.utm_subid }}"},
                                {"fieldId": "company", "fieldValue": "={{ $json.full_json.company }}"},
                                {"fieldId": "email", "fieldValue": "={{ $json.full_json.email }}"},
                                {"fieldId": "ip_address", "fieldValue": "={{ $json.full_json.ip_address }}"},
                            ]
                        },
                    },
                    "credentials": {
                ...
                },
                {
                    ...
                    "parameters": {
                        ...
                        "additionalFields": {
                            "customProperties": {
                                "customProperty": [
                                    {"key": "ip_address", "value": "={{ $('Add To Google Sheet').item.json.ip_address }}"},
                                    {"key": "brand", "value": "={{ $('Add To Google Sheet').item.json.full_json.brand }}"},
                                    {"key": "consent", "value": "={{ $('Add To Google Sheet').item.json.full_json.consent }}"},
                                    {"key": "site", "value": "={{ $('Add To Google Sheet').item.json.full_json.site }}"},
                                    {"key": "url", "value": "={{ $('Add To Google Sheet').item.json.full_json.url }}"},
                                    {"key": "referrer", "value": "={{ $('Add To Google Sheet').item.json.full_json.referrer }}"},
                                    {"key": "utm_source", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_source }}"},
                                    {"key": "utm_campaign", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_campaign }}"},
                                    {"key": "utm_medium", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_medium }}"},
                                    {"key": "utm_content", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_content }}"},
                                    {"key": "utm_referrer", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_referrer }}"},
                                    {"key": "state", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_state }}"},
                                    {"key": "zip_code", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_zip_code }}"},
                                    {"key": "leadid", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_leadid }}"},
                                    {"key": "subid", "value": "={{ $('Add To Google Sheet').item.json.full_json.utm_subid }}"},
                                    {"key": "company", "value": "={{ $('Add To Google Sheet').item.json.full_json.company }}"},
                                    # New lifecycle attributes
                                    {"key": "content_interest", "value": "={{ $('Compute Lifecycle Attributes').item.json.content_interest }}"},
                                    {"key": "state_resolved", "value": "={{ $('Compute Lifecycle Attributes').item.json.state_resolved }}"},
                                    {"key": "lifecycle_stage", "value": "={{ $('Compute Lifecycle Attributes').item.json.lifecycle_stage }}"},
                                    {"key": "welcome_flow_started", "value": "={{ $('Compute Lifecycle Attributes').item.json.welcome_flow_started }}"},
                                    {"key": "articles_received", "value": "={{ $('Compute Lifecycle Attributes').item.json.articles_received }}"},
                                    {"key": "articles_received_list", "value": "={{ $('Compute Lifecycle Attributes').item.json.articles_received_list }}"},
                                ],
                            },
                            "email": "={{ $('Add To Google Sheet').item.json.full_json.email }}",
                            "createdAt": "={{ $('Add To Google Sheet').item.json.timestamp }}",
                        },
                    },
                    "credentials": {
                        "customerIoApi": {
                ...
                },
                {
                    ...
                    "parameters": {
                        "respondWith": "json",
                        "responseBody": '{ "ok": true, "received_at": "={{$now}}" }',
                        "options": {},
                    },
                },
            ],
            "connections": {
                "Subscribe Webhook": {
                    "main": [
                        [{"node": "Verify Webhook Request", "type": "main", "index": 0}]
                    ]
                },
                "Verify Webhook Request": {
                    "main": [
                        [{"node": "Successful - Set Data Fields", "type": "main", "index": 0}],
                        [{"node": "Failed Attempt", "type": "main", "index": 0}],
                    ]
                },
                "Successful - Set Data Fields": {
                    "main": [
                        [{"node": "Compute Lifecycle Attributes", "type": "main", "index": 0}]
                    ]
                },
                "Compute Lifecycle Attributes": {
                    "main": [
                ...
                "Update Supabase": {
                    "main": [
                        [{"node": "Update Customer.io", "type": "main", "index": 0}]
                    ]
                },
                "Update Customer.io": {
                    "main": [
        ...
        print("Creating 'Saverwell Lifecycle Subscribe' workflow...")
        ...
        n8n_url = settings.n8n_base_url.replace("/api/v1", "")
